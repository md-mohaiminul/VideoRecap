...
def main(args):
    ...
    args.max_gen_tokens = 256
    caption_dataset = VideoCaptionDataset(args, transform = train_transform, is_training=True, force_len = args.force_len)
    ...
    if 'clip' in args.unify_type:
        ...
        # if args.unify_type=='caption+clip':
        #     args.metadata = '/checkpoint/mohaiminul/VideoCapHier/datasets/clip_summeries_train+pseudo.pkl'
        # elif args.unify_type=='caption+clip+video':
        args.metadata = '/checkpoint/mohaiminul/VideoCapHier/datasets/clip_summeries_train_new.pkl'
        ...
        args.max_gen_tokens = 256
        if args.hier_type == 'recur':
            args.text_feature_type = 'token'
            args.num_text_feat = 512
        clip_dataset = VideoCaptionDataset(args, transform = None, is_training=True, force_len = args.force_len)
    ...
    if 'video' in args.unify_type:
        args = copy.deepcopy(args)
        #args.metadata = '/checkpoint/mohaiminul/VideoCapHier/datasets/video_summaries_rest/video_summeries_train+pseudo_v2.json'
        args.metadata = '/checkpoint/mohaiminul/VideoCapHier/datasets/video_summaries_rest/video_summaries_train_v1+v2_with_pred.json'
        args.dataset = 'video_summary'
        args.video_feature_type='cls'
        args.video_feature_path = '/checkpoint/mohaiminul/VideoCapHier/datasets/features/videos'
        args.num_video_feat = 512
        args.max_gen_tokens = 256
        if args.hier_type == 'recur':
            args.text_feature_type = 'token'
            args.num_text_feat = 512
        video_dataset = VideoCaptionDataset(args, transform = None, is_training=True, force_len = args.force_len)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.unify_type=='caption+clip':
            train_caption_clip(caption_loader, clip_loader, model, optimizer, scaler, epoch, lr_schedule, args)
        elif args.unify_type=='caption+clip+video':
            train_caption_clip_video(caption_loader, clip_loader, video_loader, model, optimizer, scaler, epoch, lr_schedule, args)
        else:
...
#caption+clip   
def train_caption_clip(caption_loader, clip_loader, model, optimizer, scaler, epoch, lr_schedule, args):
    iters_per_epoch = len(clip_loader) // args.update_freq
    model.train()
    start = time.time()
    loader_types = ['caption', 'clip']
    for data_iter, (captions, clip) in enumerate(zip(caption_loader, clip_loader)):
        loader_cnt = 0
        for samples in (captions, clip):
            loader_cnt += 1
            # for k in samples:
            #     print(loader_types[loader_cnt-1], data_iter, k, samples[k].shape)
            # continue
            ...
            if optim_iter % args.print_freq == 0:
                ...
                if dist_utils.is_main_process():
                    ...
                    print("Epoch", epoch, ":iter", data_iter, "/", iters_per_epoch, loader_types[loader_cnt-1], "loss:", loss.item(), ", time passed", tr)
                    

#caption+clip+video   
def train_caption_clip_video(caption_loader, clip_loader, video_loader, model, optimizer, scaler, epoch, lr_schedule, args):
    iters_per_epoch = len(clip_loader) // args.update_freq
    model.train()
    start = time.time()
