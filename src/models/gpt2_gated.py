...
def augment_gpt2_config(config, cross_attn_freq=1, gated_xattn=True, use_lora = False, 
                        lora_attn_dim=128, encoder_width=None):
    ...
    new_config.use_lora = use_lora
    new_config.lora_attn_dim = lora_attn_dim
    if encoder_width is None:
        encoder_width = config.hidden_size
    new_config.encoder_width = encoder_width
...
class GPT2Attention(nn.Module):
    def __init__(self, config, is_cross_attention=False, layer_idx=None):
        ...
        if self.is_cross_attention:
            if config.use_lora:
                self.lora_query = lora.Linear(self.embed_dim, self.embed_dim, r=config.lora_attn_dim)
                self.lora_key = nn.Linear(self.embed_dim, self.embed_dim)
                self.lora_value = lora.Linear(self.embed_dim, self.embed_dim, r=config.lora_attn_dim)
                
            self.c_attn = Conv1D(2 * self.embed_dim, self.encoder_width)
...
class SqReLU(nn.Module):
    ...
    def _sqrelu_python(self, input: torch.Tensor) -> torch.Tensor:
        ...
        #return torch.pow(self.l_relu(input), 2)
        #return torch.pow(nn.functional.gelu(input), 2)
...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def freeze_lm_entire(self):
        for n, p in self.named_parameters():
            p.requires_grad = False
        print("Freeze the entire LM including CA")
    
    def freeze_except_lora(self):
        for n, p in self.named_parameters():
            if 'lora' not in n:
                p.requires_grad = False
        print("Freeze all but lora weights")
    ...
    def freeze_lm_weights(self):
        ...
        print("Freeze the pretrained parts in LM: {}".format(len(freeze_list)))
        print("Learn the rest parts in LM: {}".format(len(unfreeze_list)))
