...
def main(args):
    ...
    # min = 60
    # old_args.num_video_feat = int(15*min)
    # old_args.max_gen_tokens = 77

    # old_args.video_mapper_type = 'perciever'
    # old_args.text_mapper_type = 'perciever'
    ...
    old_args.resume = 'pretrained_models/videorecap/videorecap_clip.pt'
    print("=> Creating model")
    model = VideoRecap(old_args)
    model = model.to(args.device)

    old_args.resume = 'pretrained_models/videorecap/videorecap_segment.pt'
    model2 = VideoRecap(old_args)
    ...
    for (p1, p2) in zip(model.parameters(), model2.parameters()):
        if p1.requires_grad:
            try:
                l = cka.linear_CKA(p1.cuda(), p2.cuda())
                k = cka.kernel_CKA(p1.cuda(), p2.cuda())
                print(l.item(), k.item(), cka.linear_CKA(p1.cuda(), p1.cuda()).item(), cka.kernel_CKA(p1.cuda(), p1.cuda()).item())
                lin += l
                ker += k
                t += 1
            except:
